...
##Merge well count back into CES file

```{r}

ces_new_well <- merge(ces, new_well_count, by = "tract") %>% 
  rename(new_wells = n)

ces_new_well$

ces_active_well <- merge(ces, active_well_count, by = "tract") %>% 
...
#read in visual for CalEnvironScreen with polygons set the Race_PERCENTILE by Census Tract
tm_shape(ces_new_well) +
  tm_polygons(c("CIscoreP", "new_wells")) +
  tm_facets(sync = TRUE) +
  tmap_options(limits = c(facets.view = 2))

```

#View CIS Scores and Active Wells
...
#read in visual for CalEnvironScreen with polygons set the Race_PERCENTILE by Census Tract
tm_shape(ces_active_well) +
  tm_polygons(c("CIscoreP", "active_wells")) +
  tm_facets(sync = TRUE) +
  tmap_options(limits = c(facets.view = 2))
