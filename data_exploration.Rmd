...
#filter out only ACTIVE wells
well_location_ACTIVE <- well_location_table %>% 
  filter(WellStatus == "Active") 

#filter out only NEW wells
well_location_NEW <- well_location_table %>% 
  filter(WellStatus == "New") 


unique(well_location_ACTIVE$WellStatus)
#Canceled Active   New      Plugged  Unknown  Idle     Buried   Abeyance

unique(well_location_table)
#"Latitude"   "Longitude" 
...
ces_shapefile <- st_as_sf(ces_shapefile)
plot(ces_shapefile, graticule = st_crs(4326), key.pos = 1)


plot(well_location_spatial, pch = 19, col = "black", add = TRUE)



st_crs(ces_shapefile) <- "+proj=lcc +lat_1=48 +lat_2=33 +lon_0=-100 +ellps=WGS84"
st_crs(well_location_spatial) <- ces_shapefile



intersect <- st_intersection(ces_shapefile, well_location_NEW)
...
#convert well_location_NEW to SpatialPointData
coordinates(well_location_NEW) <- c("Longitude","Latitude")

#re_read CES as SpatialPolygonData

ces_shapefile2 <- readOGR(file.choose())

#set projection 

proj4string(well_location_NEW) <- proj4string(ces_shapefile2)

#count points within CES polygons
results <- over(well_location_NEW, ces_shapefile2)

table(results$Name_1)
...
#Using poly.count() function

poly.counts(well_location_NEW, ces_shapefile2) -> results2

setNames(results2, ces_shapefile2@data)
